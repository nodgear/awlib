...
local function LogPrint( err, module, file, line, realmcolor, typecolor, ...)
    if !module or !file then
        if CLIENT then
            chat.AddText("[AWESOME] " .. err, SPrint(...))
        else
            MsgC("\n[AWESOME] " .. err, SPrint(...) .."\n")
        end
        return
    ...
    if CLIENT then
        chat.AddText(realmcolor,"[AWESOME]".."["..module.."] ", typecolor, err, SPrint(...) .. "~> @" .. file ..":".. line .."\n")
    else
        MsgC(realmcolor,"[AWESOME]".."["..module.."] ", typecolor, err, SPrint(...), " ~> @" .. file ..":".. line .."\n")
    end
...
function Aw:Log(err, type, ...)
    ...
    sModule = string.upper(sModule[iModuleStart])
